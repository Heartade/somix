...
pub fn compose(props: &Props) -> Html {
    ...
    let send_onclick = {
        ...
        Callback::from(move |_| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                let body = compose_textarea_state.deref().to_string();
                let event_id = match reply_to {
                    Some(post) => reply_to_message(post, body).await.unwrap(),
                    None => send_message(room_id, body).await.unwrap(),
                };
                get_posts().await.unwrap();
                compose_textarea_state.set("".to_string());
    ...
    html! {
        <div class="flex justify-center w-full">
            <div class={ match props.clone().reply_to { Some(_) => "w-full".to_string(), None => "w-2/3 md:w-3/5 lg:w-1/2 xl:w-2/5".to_string() } }>
                <div class="flex flex-col gap-2 border border-tuatara-400 bg-tuatara-600 rounded p-2">
                    {
                        if *loading_state {
                            html! {}
                        } else {
